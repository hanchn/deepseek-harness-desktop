...
> [#43](https://github.com/web-casa/DeepSeek-Harness-Desktop/issues/43) 已知兼容性问题：AppImage
...
>
> ```bash
> GDK_BACKEND=wayland ./<你的-AppImage-文件>.AppImage
> ```
>
...
| 📊 **用量与余量** | 控制器角落以水印常驻显示今日 token 消耗、调用次数、花费估算与账户余量（右下/右上/隐藏可切换）；点击展开按模型拆分与近 30 天历史。token 数取自本机会话日志里服务商回报的用量，花费按官方价格表分高峰/低谷估算 |
...
控制器的「用量与余量」以**角落水印**常驻显示（默认右下角，可在页首切换为右上角
或隐藏），点击水印展开明细面板。它把本机已经存在的数据翻译成可读数字，不上传
任何用量：

- **token 与调用次数**来自 `<DSH_HOME>/sessions/**/session.v3.jsonl.zstd`：Harness
  把每个事件写成一个独立 zstd 帧，其中 `assistant/message` 事件带服务商回报的
  一次计费用量（未命中缓存输入 / 命中缓存输入 / 输出，输出含思维链）。Desktop
  只读这些日志，按下单时刻归属到**本地自然日**与模型，因此换时区、跨天长时间
  会话都不会算错日期。种子/派生会话中被重放的上游事件按会话创建时间排除，
  避免把父会话的调用重复计一遍。
- **花费是估算，不是账单**：没有任何接口提供本机账单，Desktop 用内置的官方价格表
  （卡片上标注币种与读取日期）对每次调用按其**实际所处的时段**计价——高峰
  （UTC 周一至周五 `01:00–04:00`、`06:00–10:00`，即北京时间 `09:00–12:00`、
  `14:00–18:00`）单价是低谷的两倍。价格表没收录的模型只统计 token，显示为 `—`
  而不是 `$0.00`，并单列未计价次数。
- **账户余量**是唯一联网的部分：Desktop 用 Harness 凭据存储
  （`<DSH_HOME>/.credentials.yaml` 的 `refs`，环境变量优先）里的 Key 调用一次
  DeepSeek 官方 `GET /user/balance`，只回传余额数字；Key 不落日志、不回传 WebView。
  未配置 Key、Key 被拒、超时等情况分别给出可读提示。
- **历史**默认近 30 天（含空白天，便于看出趋势），点击某一天即可查看该日的
  按模型明细；卡片同时显示汇总了多少个会话日志、有多少条被排除的重放事件。

价格与口径随版本更新，最新单价以
[DeepSeek 官方定价页](https://api-docs.deepseek.com/quick_start/pricing)为准。
